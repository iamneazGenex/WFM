...
def getAgentHourlyPerformanceMappingWithRosterByMonth(month, year, skill):
    ...
    # We will fetch all the rosters for the given month and skill at once to minimize DB calls.
    rosters = Roster.objects.filter(
        agenthourlyperformance__date__month=month,
        agenthourlyperformance__date__year=year,
        agenthourlyperformance__skill=skill,
    ...
    # Fetch all agent hourly performances in one query
    agentHourlyPerformances = AgentHourlyPerformance.objects.filter(
    ...
    # Initialize shift count and absent for each date in the month
    for item in tableData:
        date = item["date"]
        shiftCount = 0
        absent = 0

        # Filter the agent hourly performances for the current date
        day_performances = agentHourlyPerformances.filter(date=date)

        # Use a set to track unique rosters
        unique_roster_ids = set(day_performances.values_list("roster_id", flat=True))

        # Aggregate the shift count and absenteeism based on the roster data
        for roster_id in unique_roster_ids:
            roster_data = roster_mapping.get(roster_id)
            if roster_data:
                shiftCount += roster_data["shift_count"]
                absent += roster_data["is_absent"]
