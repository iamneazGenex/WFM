...
def getAgentHourlyPerformanceMappingWithRosterChange(date, skill):
    ...
    # Initialize dictionaries to store shift count and absence for each hour
    hourly_data = {hour: {"shiftCount": 0, "absent": 0} for hour in range(24)}

    # Iterate over the fetched records and aggregate the data by hour
    for item in agentHourlyPerformances:
        hour = item["hour"]
        shift_count = item["roster__shiftLegend__shift_count"] or 0
        absent = item["roster__is_absent"] or 0

        # Sum shift counts and absences for each hour
