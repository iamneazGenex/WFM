...
def getAgentHourlyPerformanceMappingWithRosterChange(date, skill):
    ...
    for hour in hour_list:
        ...
        #     .filter(roster_id__isnull=False)
        #     .values("roster_id")  # Only select `roster_id` initially
        #     .annotate(
        #         shiftCount=Sum("roster__shiftLegend__shift_count"),
        #         absent=Sum(
        #             Case(
        ...
        #         ),
        #     )
        #     .distinct()  # Ensure we get unique rosters
        # )
        unique_roster_data = (
            AgentHourlyPerformance.objects.filter(date=date, skill=skill, hour=hour)
            .filter(roster_id__isnull=False)
        ...
        for item in unique_roster_data:
            print(
                f"Roster ID: {item['roster_id']} | Shift Count: {item['roster__shiftLegend__shift_count']}"
            )

        # Logging the count of unique roster entries
        logger.info(f"unique_roster_data count: {unique_roster_data.count()}")

        shiftCount = 0
        absent = 0

        # Loop through and sum the shift count and absence
        for item in unique_roster_data:
            shiftCount += item["roster__shiftLegend__shift_count"] or 0
            absent += item["roster__is_absent"] or 0  # Avoid None by defaulting to 0
        print(f"ShiftCount: {shiftCount} | Absent: {absent}")
