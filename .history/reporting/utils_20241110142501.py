...
def getAgentHourlyPerformanceMappingWithRosterChange(date, skill):
    ...
    # Iterate over hours
    for hour in hour_list:
        # Aggregate the data directly from the database
        unique_roster_data = (
            AgentHourlyPerformance.objects.filter(date=date, skill=skill, hour=hour)
            .filter(roster_id__isnull=False)
            .values(
                "roster_id", "roster__shiftLegend__shift_count", "roster__is_absent"
            )
            .distinct()  # Ensure no duplicate rosters are counted
        )

        # Initialize shift count and absence count
        shiftCount = 0
        absent = 0

        # Loop through the aggregated results to calculate shift count and absent
        for item in unique_roster_data:
            shiftCount += item["roster__shiftLegend__shift_count"] or 0
            absent += item["roster__is_absent"] or 0

        # Log the results for each hour
        logger.info(
            f"Hour: {hour} | unique_roster_data count: {len(unique_roster_data)}"
        )
        logger.info(f"ShiftCount: {shiftCount} | Absent: {absent}")

        # Update the mapping for the current hour
        agentHourlyPerformanceMapping[hour] = {
            "shiftCount": shiftCount,
            "absent": absent,
        }

        # Optionally, print if needed for debugging
        print(f"Hour: {hour} | ShiftCount: {shiftCount} | Absent: {absent}")
