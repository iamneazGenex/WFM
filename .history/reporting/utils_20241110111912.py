...
def getAgentHourlyPerformanceMappingWithRoster(date, skill):
    ...
    try:
        for hour in hour_list:
            shiftCount = 0
            absent = 0
            logger.info("Processing hour: %d", hour)
            ...
            logger.info(
                "Retrieved %d records for hour %d",
                agentHourlyPerformances.count(),
                hour,
            )

            # Get distinct roster IDs for the current hour
            unique_roster_ids = (
            ...
            logger.info(
                "Found %d unique roster IDs for hour %d", len(unique_roster_ids), hour
            )
            ...
            for roster_id in unique_roster_ids:
                ...
                logger.info(
                    "Roster ID: %d | Shift Count: %d | Absent: %d",
                    roster.id,
                    roster.shiftLegend.shift_count,
                    roster.is_absent,
                )

            # Update the agentHourlyPerformanceMapping dictionary with aggregated data for this hour
            agentHourlyPerformanceMapping[hour] = {
                "shiftCount": shiftCount,
                "absent": absent,
            }
            logger.info(
                "Hour %d | Shift Count: %d | Absent: %d", hour, shiftCount, absent
            )
