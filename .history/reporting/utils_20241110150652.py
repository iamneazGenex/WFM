...
def getAgentHourlyPerformanceMappingWithRosterByMonth(month, year, skill):
    ...
    try:
        # Fetch all AgentHourlyPerformance for the given month and skill
        agentHourlyPerformances = AgentHourlyPerformance.objects.filter(
            date__month=month, date__year=year, skill=skill
        ).select_related("roster__shiftLegend")
        ...
        # Pre-fetch all rosters for the given month
        ...
        # Create a mapping of roster IDs to shift counts and absenteeism data
        roster_mapping = {
            roster.id: {
        ...
        # Process the agent hourly performance records and aggregate the data by date
        # Initialize a defaultdict to store shift counts and absenteeism for each date
        daily_performance_aggregation = defaultdict(
            lambda: {"shiftCount": 0, "absent": 0}
        )

        # Iterate through all agent hourly performance records
        for performance in agentHourlyPerformances:
            date = performance.date
            roster_id = performance.roster.id

            # Aggregate the shift count and absenteeism based on the roster data
            if roster_id in roster_mapping:
                daily_performance_aggregation[date]["shiftCount"] += roster_mapping[
                    roster_id
                ]["shift_count"]
                daily_performance_aggregation[date]["absent"] += roster_mapping[
                    roster_id
                ]["is_absent"]

        # Update the final mapping for each date
        for item in tableData:
            date = item["date"]
        ...
        logger.error(f"Error processing data for month {month}-{year}: {str(e)}")
