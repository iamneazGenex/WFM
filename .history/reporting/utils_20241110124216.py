...
def getAgentHourlyPerformanceMappingWithRosterExact(date, skill):
    ...
    """
    start_time = time.time()
    logger.info("Fetching Agent Hourly Performance mapping for date: %s, skill: %s", date, skill)

    # Initialize the results dictionary with placeholders for each hour
    agentHourlyPerformanceMapping = {hour: {"shiftCount": 0, "absent": 0} for hour in range(24)}
    ...
    try:
        ...
        for ahp in agentHourlyPerformances:
            ...
            if roster and roster.id not in hourly_data[hour]:
                hourly_data[hour][roster.id] = True  # Store seen rosters to avoid duplicates
        ...
        for hour, data in hourly_data.items():
            ...
            logger.debug("Hour %d | Shift Count: %d | Absent: %d", hour, data["shiftCount"], data["absent"])

        logger.info("Successfully created Agent Hourly Performance mapping")

    except Exception as e:
        logger.error("Error fetching Agent Hourly Performance mapping for date: %s, skill: %s - %s", date, skill, e)

    end_time = time.time()
    elapsed_time = end_time - start_time
    logger.info("Execution time for getAgentHourlyPerformanceMappingWithRosterExact: %.4f seconds", elapsed_time)
