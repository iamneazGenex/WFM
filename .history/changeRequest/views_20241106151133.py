    ...
    if request.method == "POST":
        ...
        if form1.is_valid():
            ...
            try:
                ...
                return render(request, template, context)
            # Create ShiftTimeTrading instance
            try:
                shift_time_trading_exists = ShiftTimeTrading.objects.filter(
                    Q(requestor=requestor),
                    Q(requestee=requestee),
                    Q(requestor_swap_roster=requestorRoster),
                    Q(requestor_start_time=requestorRoster.start_time),
                    Q(requestor_end_time=requestorRoster.end_time),
                    Q(requestee_swap_roster=requesteeRoster),
                    Q(requestee_start_time=requesteeRoster.start_time),
                    Q(requestee_end_time=requesteeRoster.end_time),
                    Q(trading_status=["in process", "accepted"]),
                ).exists()

                if shift_time_trading_exists:
                    logging.error("Trading already exists")
                    messages.error(request, "Trading already exists")
                    context["form1"] = form1
                    return render(request, template, context)

                shift_time_trading_instance = ShiftTimeTrading(
                    requestor=requestor,
                    requestee=requestee,
                ...
                shift_time_trading_instance.save()
                messages.success(request, "Shift trade request successfully submitted.")
                return redirect(PageInfoCollection.SHIFTTIMETRADING_VIEW.urlName)

            except Exception as e:
                logger.error(f"Error creating shift trade request: {e}")
                messages.error(
                    request, "An error occurred while creating the shift trade request."
                )
        ...
        form1 = ShiftTimeTradingForm(
            employee=requestor, swapDateID=None, work_rule=workRule, requesteeID=None
        )
        context["form1"] = form1
...
#     GroupEnum.wfm, GroupEnum.supervisor, GroupEnum.employee, GroupEnum.mis_group_1
# )
# def createshiftTimeTradingOLD(request):
#     template = "changeRequest/shiftTimeTrading/create.html"
#     breadCrumbList = [
#         PageInfoCollection.SHIFTTIMETRADING_VIEW,
#         PageInfoCollection.SHIFTTIMETRADING_CREATE,
...
def get_roster_details(request):
    ...
    print(requestee)
    swap_roster = Roster.objects.get(id=swap_date_id)
    print(swap_roster)
    try:
        roster_entry = Roster.objects.get(
            employee=requestee, start_date=swap_roster.start_date
        )
        data = {
    ...
    if request.user.is_Supervisor():
        ...
        if record.supervisor_approval_status == None:
            swapResult = swapRosterTimes(
                record.requestor_swap_roster.id, record.requestee_swap_roster.id
            )
            if swapResult is True:
                ...
                errorMessage = "Internal error occurred"
                logging.warning(f"|Failed| Swap Failed |id:{id}|")
        else:
            errorMessage = "Decision already taken"
            logging.warning(f"|Failed| {errorMessage} |id:{id}|")
...
class ShiftTimeTradingListJson(BaseDatatableView):
    ...
    def get_initial_queryset(self):
        ...
        now = datetime(2024, 1, 1, 14, 00, 45)
        if self.request.user.is_Employee():
            ...
            return ShiftTimeTrading.objects.all()
        ...
        if self.request.user.is_Employee():
            for item in qs:
                ...
                row = {
                    ...
                    "requestor_end_time": item.requestor_end_time.strftime("%H:%M %p"),
                    "requestee_start_time": item.requestee_start_time.strftime(
                        "%H:%M %p"
                    ),
                    "requestee_end_time": item.requestee_end_time.strftime("%H:%M %p"),
