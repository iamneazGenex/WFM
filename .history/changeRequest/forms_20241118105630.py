...
class DayOffTradingForm(forms.Form):
    ...
    def __init__(self, employee, swapDateID, tradeDateID, requesteeID, *args, **kwargs):
        ...
        currentDate = datetime(2024, 11, 1, tzinfo=timezone.utc)
...
class ShiftTimeTradingForm(forms.Form):
    ...
    def validate_swap(self, employee_roster, requestee_roster):
        ...
        self.check_if_same_shift_time(temp_employee_roster, temp_requestee_roster)
    ...
    def check_shift_gap(self, roster):
        logger.info(
            f"Checking Shift Gap of {roster.employee.user.name} | roster: {roster}"
        )
        previous_roster = (
    ...
    def check_female_shift_times(self, employee, roster):
        logger.info(
            f"Checking Female Shift Time of {roster.employee.user.name} | roster: {roster}"
        )
    ...
    def check_prohibited_shift_end(self, roster):
        logger.info(
            f"Checking Prohibited Shift End Time of {roster.employee.user.name} | roster: {roster}"
        )
        if (
    ...
    def check_consecutive_working_days(self, roster):
        logger.info(
            f"Checking consecutive working days of {roster.employee.user.name} | roster: {roster}"
        )
        # Define date range to fetch rosters around the swap and trade dates
        days_before = roster.start_date - timedelta(
            days=self.work_rule.consecutive_working_days
        )
        ...
        for roster_entry in roster_dates:
            ...
            if consecutive_count > self.work_rule.consecutive_working_days:
                error_message = f"Consecutive working days ({consecutive_count}) exceed the limit ({self.work_rule.consecutive_working_days}) for {roster.employee.user.name}"
                logger.info(error_message)
                raise ValidationError("\n".join(error_message))

    def check_if_same_shift_time(self, requestor_roster, requestee_roster):
        if (
            requestor_roster.start_time == requestee_roster.start_time
            and requestor_roster.end_time == requestee_roster.end_time
        ):
            error_message = f"Same shift time Swaping Not allowed"
            logger.info(error_message)
            raise ValidationError("\n".join(error_message))
